...
test('Test the encoding enum', async (t) => {
  ...
  await t.test('It should throw an exception for an unknown value', () => {
    try {
      ...
      assert.strictEqual(error.message, 'Cannot set property keys of [object Object] which has only a getter')
    }
  })
  ...
  await t.test('It should throw an exception for an unknown value', () => {
    try {
      ...
      assert.strictEqual(error.message, 'Cannot set property values of [object Object] which has only a getter')
    }
  })
  ...
  await t.test('It should also work with multiple options', () => {
    ...
    assert.strictEqual(JSON.stringify(Example), '{"test1":1,"test2":2,"test3":3}')
